...
# scale used in optimization for reflection term
# for ocean4 clip 0, 1, refl_scale = 1/16
# really a hack to change the value...
refl_scale = 1 / 128

def main():
    
    dir = '/n/fs/shaderml/deeplab-pytorch/result/clip06'
    highlight_dir = '/n/fs/shaderml/drone_videos/drone_frames/ocean3_06/highlight'
    orig_img_dir = '/n/fs/shaderml/drone_videos/drone_frames/ocean3_06'
    out_dir = 'optimize_debug/clip06'
    ...
    with session.as_default():
        ...
        if not read_1st_round_from_file:
            keyframe_params = np.empty([len(keyframes), dims])
            all_frames_seg = np.empty([nframes, 2])
            all_frames_par = np.empty([nframes, dims])
            for idx in range(nframes):
                ...
                def opt_func(x):
                    ...
                    loss2 = np.mean(diff_refl ** 2.0) * refl_scale
                ...
                if idx % 10 == 0:
                    ...
                    is_sea_optimized = np.argmin(current_seg[:, :, 0], axis=0)
                    fig = plt.figure()
                    plt.imshow(orig_img)
                    plt.plot(np.squeeze(line_X), is_sea_optimized)
                    fig.savefig(os.path.join(out_dir, filename_only + '_optimized_seg.png'))
                    plt.close(fig)

            #np.save('sequence_keyframe_par.npy', keyframe_params)
            np.save(os.path.join(out_dir, 'all_frames_seg.npy'), all_frames_seg)
            np.save(os.path.join(out_dir, 'all_frames_par.npy'), all_frames_par)
