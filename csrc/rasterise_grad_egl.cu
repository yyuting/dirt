...
__global__ void upload_background_copy(cudaSurfaceObject_t dest_surface, TTypes<float, 4>::ConstTensor const src_tensor, int const frames_per_row, dim3 const total_threads)
{
...
void launch_background_upload_copy(
    ...
    upload_background_copy<<<config.block_count, config.thread_per_block, 0, device.stream()>>>(
        dest_surface,
        src,
...
__global__ void download_pixels_copy(TTypes<float, 4>::Tensor pixels, cudaSurfaceObject_t const src_surface, int const frames_per_row, dim3 const total_threads)
{
...
void launch_pixels_download_copy(
    ...
    download_pixels_copy<<<config.block_count, config.thread_per_block, 0, device.stream()>>>(
        dest,
        src_surface,
